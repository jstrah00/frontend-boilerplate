...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed z-50 grid bg-background shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
        // Centered modal with responsive sizing
        "left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] w-[calc(100%-1rem)] max-w-lg rounded-lg border p-4 gap-3",
        "max-h-[calc(100vh-2rem)] overflow-y-auto",
        // Desktop: larger padding
        "sm:w-full sm:p-6 sm:gap-4 sm:max-h-[85vh]",
...
const DialogHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col space-y-1 sm:space-y-1.5 text-center sm:text-left pr-6",
      className
    )}
    {...props}
  />
